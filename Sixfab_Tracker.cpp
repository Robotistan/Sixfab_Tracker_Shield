...
const char* SixfabTracker::enterPinCode(const char *pinCode)		// MSMORKAN 02.2021
{
  const char prefix = "AT+CPIN=";
  return sendATComm(prefix+pinCode,"OK");
}
...
String SixfabTracker::readSMS(uint8_t index)			// MSMORKAN 02.2021
{
	
  String gsmBuffer = "";
  sendATComm("AT+CMGF=1","OK");
  ...
  if (gsmBuffer.indexOf("CMGR:")!=-1){
    return gsmBuffer;
  }
  else return "";
}

void SixfabTracker::deleteAllSMS(void) 					// MSMORKAN 02.2021
{
  sendATComm("at+cmgd=1,2","OK");
}
